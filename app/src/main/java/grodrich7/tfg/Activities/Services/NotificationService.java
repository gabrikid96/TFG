...
public class NotificationService extends Service {
                        ...
                        for (DataSnapshot groupSnapshot : userSnapshot.getChildren()) {
                            for (DataSnapshot drivingDataSnapshot : groupSnapshot.getChildren()){
                                if (drivingDataSnapshot.getKey().equals(FirebaseAuth.getInstance().getUid())){
                                    ...
                                    if (usersData.containsKey(drivingDataSnapshot.getKey())){
                                        DrivingData old = usersData.get(drivingDataSnapshot.getKey());
                                        old.merge(drivingData);
                                        usersData.put(drivingDataSnapshot.getKey(), old);
                                    }else{
                                        usersData.put(drivingDataSnapshot.getKey(), drivingData);
                                    }
                                }
                            }
                        }
    ...
    private void addNotification(String key, String name){
        ...
        NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(this)
                ...
                .setContentTitle(name + " is driving now!")
                ...
                .addAction(new NotificationCompat.Action(
                        R.mipmap.driving_on,
                        this.getString(R.string.finish),
        ...
        //this.startForeground(1337,notificationBuilder.build());
        ...
        mBuilder.setSmallIcon(R.mipmap.driving_on);
        mBuilder.setContentTitle(name + " is driving now!");
        mBuilder.setContentText("Tap to view");
                                                ...
                                                if (drivingDataSnapshot.getKey().equals(FirebaseAuth.getInstance().getUid())){
                                                    ...
                                                    if (old == null){
                                                        if (drivingData.isDriving() != null && drivingData.isDriving().booleanValue()){
                                                            Log.d("NOTIFICATION_DRIVING", drivingDataSnapshot.getKey() + " is now driving");
                                                            usersData.put(drivingDataSnapshot.getKey(), drivingData);
                                                            addNotification(drivingDataSnapshot.getKey(), "User");
                                                        }
                                                    }else{
